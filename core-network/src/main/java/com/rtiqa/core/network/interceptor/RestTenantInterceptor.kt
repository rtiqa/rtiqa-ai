...
class RestTenantInterceptor(private val sessionStore: RestSessionStore) : Interceptor {
    companion object {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (originalRequest.header(NO_TENANT_HEADER) != null) {
            val newRequest = originalRequest.newBuilder().removeHeader(NO_TENANT_HEADER).build()
            return chain.proceed(newRequest)
        }
        val requestBuilder = originalRequest.newBuilder()
        val tenantId = sessionStore.getActiveOrganizationId()
        if (!tenantId.isNullOrBlank()) requestBuilder.header(HEADER_TENANT_ID, tenantId)
